...
export async function fetchLastWeekData(): Promise<WeeklyData> {
  const response = await fetch(
    "http://localhost:8080/daily.json?tz=America/Los_Angeles&q=min_outTemp&q=max_outTemp&q=avg_outTemp&q=min_outHumi&q=avg_outHumi&q=max_outHumi&q=max_gustspeed&q=avg_avgwind&q=max_gustspeed&q=avg_rainofhourly",
  )
  const body = await response.json()
...
export async function fetchHourlyData(date: string): Promise<DailyData> {
  const response = await fetch(
    `http://localhost:8080/hourly.json?tz=America/Los_Angeles&date=${date}&q=min_outTemp&q=max_outTemp&q=min_outHumi&q=max_outHumi&q=max_gustspeed&q=avg_avgwind&q=max_gustspeed&q=avg_rainofhourly&q=avg_outHumi&q=avg_outTemp`,
  )
  const body = await response.json()
