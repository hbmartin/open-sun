...
export default function HourlyRow({
  hour,
  minTemp: minTemporary = 70,
  maxTemp: maxTemporary = 95,
}: {
  hour: HourlyData
  minTemp?: number
  maxTemp?: number
  ...
  const temporaryRange = maxTemporary - minTemporary
  const temporaryPosition = ((hour.temp - minTemporary) / temporaryRange) * 100
