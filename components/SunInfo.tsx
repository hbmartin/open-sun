  ...
  return (
    <div className="px-4 py-3 text-center">
      <div className="flex items-center justify-center">
        ...
        <span className="text-gray-700 mr-4">{timesData?.dawn.getHours()}:{timesData?.dawn.getMinutes()} AM</span>
        <Rss size={16} className="text-gray-800 mr-2" />
        <span className="text-gray-700 mr-4">{new Date().getHours() % 12 || 12}:{new Date().getMinutes().toString().padStart(2, '0')} {new Date().getHours() >= 12 ? 'PM' : 'AM'}</span>
        <Sunset size={16} className="text-purple-800 mr-2" />
        <span className="text-gray-700">{timesData?.dusk.getHours()}:{timesData?.dusk.getMinutes()} PM</span>
      </div>
    </div>
  )
}
